...
class Net(nn.Module):
    def __init__(self):
        super().__init__()
        # 把 nn.Linear 里的 bias 关掉，为适配 FPGA 上的硬件实现
    ...
    def quantize_16bit(self, weight):
        # 简单模拟 16-bit 定点数量化，例如 1 位符号位，3 位整数，12 位小数
        scale = 2 ** 12
        val = torch.round(weight * scale)
        val = torch.clamp(val, -32768, 32767)
        q_weight = val / scale
        # 使用直通估计器 (Straight-Through Estimator, STE) 使梯度可以通过 torch.round 传播
    ...
    def forward(self, x):
        ...
        # 在前向传播时临时对权重进行 16 位量化限制
        w1_q = self.quantize_16bit(self.fc1.weight)
        w2_q = self.quantize_16bit(self.fc2.weight)
        ...
        for step in range(num_steps):
            ...
            cur2 = torch.nn.functional.linear(spk1, w2_q)
...
print("混淆矩阵图片已保存至 confusion_matrix.png")
